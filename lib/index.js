...
export function apply(ctx, config) {
	...
	function focusBrowser() {
		...
		const result = focusBrowserWindow({
			...
			// 总是先试切标签：切到了就不开新标签；切不到才用 openUrl 兜底 ——
			// 这样即使「有没有标签」的判断失真，结果依然正确。
			tabTitle: config.focusTabTitle,
			openUrl: guiUrl || config.webUrl,
			// 有标签就多试几次（别漏），没标签就少试几次（别白闪）
			tabSearchTries: tabAlive() ? 15 : 3,
		})
