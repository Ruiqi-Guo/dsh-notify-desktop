...
/** 包内自带的聚焦脚本。 */
export const BUNDLED_FOCUS = join(HERE, '..', 'assets', 'dsh-focus-window.ps1')

/**
 * 决定用哪个聚焦脚本：配置指定且存在就用它，否则用包内自带的。
 * @param configured - 配置项，可为空。
 * @returns 脚本绝对路径；都不存在时返回 null。
 */
export function resolveFocusScript(configured) {
	if (configured && existsSync(configured)) return configured
	return existsSync(BUNDLED_FOCUS) ? BUNDLED_FOCUS : null
}

/**
 * 把浏览器窗口拉到前台（分离进程，立即返回）。
 *
 * 与卡片同理走 `cmd / c start`：直接 `spawn('powershell.exe')` 出来的进程会
 * 「活着但没有窗口」（见 docs/engineering-notes.md 第 3 条）。
 * 刻意不用「打开 GUI 的 URL」那招 —— GUI 首页要求鉴权，实际地址带 token
 * 且随重启变化，靠 URL 匹配只会开出一个废标签页。
 *
 * @param options - `{ script, title, className }`
 * @returns `{ ok, pid }` 或 `{ ok: false, error }`
 */
export function focusBrowserWindow(options) {
	const { script, title, className, tabTitle, openUrl, tabSearchTries } = options ?? {}
	if (!script) return { ok: false, error: 'focus script not found' }
	if (!existsSync(script)) return { ok: false, error: `focus script missing: ${script}` }

	const args = ['-NoProfile', '-ExecutionPolicy', 'Bypass', '-File', script]
	if (title) args.push('-Title', String(title))
	if (className) args.push('-Class', String(className))
	// 目标标签页标题子串：聚焦脚本会切标签直到标题匹配
	if (tabTitle) args.push('-TabTitle', String(tabTitle))
	// 宿主说没有 GUI 标签可切：跳过逐个切标签（空字符串参数会被 -File 吞掉，所以用开关）
	if (tabSearchTries !== undefined) args.push('-TabSearchTries', String(tabSearchTries))
	// 没有 GUI 标签可切时用它新起一个 DSH 标签
	if (openUrl) args.push('-OpenUrl', String(openUrl))

	try {
		const child = spawn('cmd.exe', ['/c', 'start', '', '/b', 'powershell.exe', ...args], {
			stdio: 'ignore',
			windowsHide: true,
		})
		child.unref()
