...
window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const REGISTER_PATH = '/dsh-notify/register';
		const POLL_MS = 1000;

		/**
		 * 把自己的地址报给宿主。宿主需要它来切回这个标签页 ——
		 * GUI 地址带 token 且每次 `dsh web` 重新生成，只有页面自己知道；
		 * 而非浏览器进程没有别的办法切换 Chrome 的标签页（UIA 读不到，实测）。
		 */
		function reportUrl() {
			try {
				fetch(REGISTER_PATH + '?url=' + encodeURIComponent(location.href), { cache: 'no-store' }).catch(function () {});
			} catch {
				/* 忽略 */
			}
		}
		...
		function apply(ctx) {
			...
			function select(sessionId) {
				// 顺手把自己拉回前台（Chrome 通常不允许后台标签页这么做，但成本为零）；
				// 真正可靠的手段是宿主持有地址后用「打开同一 URL」切回标签页。
