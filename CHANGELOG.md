...
- **DSH 标签页已关闭时，点击卡片不再满浏览器乱切标签。** 宿主用「浏览器半边是否还在轮询」
  判断有没有 GUI 标签开着（没有标签就没人轮询，5 秒内无轮询即视为已关闭）；
  没有标签时只弹卡片、跳过切标签那一步。逐个切换的上限也从 25 次降到 15 次。
- 说明：关掉标签页**不会**停止任务 —— 任务跑在宿主进程里，浏览器只是界面客户端。

### 变更

- **DSH 标签页关闭时，点击卡片会新开一个 DSH 标签并跳到对应会话。**
  宿主用「浏览器半边是否还在轮询」区分两种情况：有标签就切回它（Ctrl+Tab 逐个找），
  没有标签就打开一个地址新起标签 —— 新标签加载后会自己轮询并消费这次点击，于是自动跳到该会话。
  地址优先用浏览器半边上报过的（带 token），没有则退回 `webUrl`（默认 `http://127.0.0.1:3080/`）。
